...
using Rotate = attach_shelf_interfaces::srv::Rotate;
...
namespace nav2_apps{
    class ApproachServiceReal : public rclcpp::Node {
        public:
            ApproachServiceReal(const rclcpp::NodeOptions & nodeOptions) : rclcpp::Node("approach_service_node", nodeOptions) {
                ...
                this -> robot_movement_publisher_ = this -> create_publisher<Twist>("/cmd_vel", this -> robot_movement_publisher_qos_);
                ...
                this -> robot_position_subscription_ = this -> create_subscription<Odometry>("/odom", this -> robot_position_subscription_qos_, [this](const Odometry::SharedPtr msg){
                ...
                this -> rotate_service_ = this -> create_service<Rotate> ("/rotate", 
                    [this](const Rotate::Request::SharedPtr request, const Rotate::Response::SharedPtr response){

                    this -> done_ = false;
                    rotate_robot__(request -> angle);
                    this -> rotate_timer_ = this -> create_wall_timer(50ms, [this](){
                        
                        if(this -> done_){
                            std::lock_guard<std::mutex> lock(rotate_mutex_);
                            rotate_cv_.notify_one();
                        }
                        
                    }, done_callback_group_);
                    std::unique_lock<std::mutex> lock(rotate_mutex_);
                    rotate_cv_.wait(lock);
                    response -> success = true;
                });   
            }
        ...
        protected:
            ...
            //rotate
            rclcpp::Service<Rotate>::SharedPtr rotate_service_;
            rclcpp::TimerBase::SharedPtr rotate_timer_;
            std::condition_variable rotate_cv_;
...
RCLCPP_COMPONENTS_REGISTER_NODE(nav2_apps::ApproachServiceReal)
